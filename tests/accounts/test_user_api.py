...
class TestUserAPITests:
    ...
    def setup(self, user):
        self.user = user
        self.client = TestClient(api.user_router)
    ...
    def test_update_token(self):
        ...
        response = self.client.put("/update/", json=payload,  headers={"Authorization": f"Bearer {self.user.api_token}"})
    ...
    def test_update_token_with_invalid_auth_token(self):
        payload = {"refresh_token": True}
        response = self.client.put("/update/", json=payload, headers={"Authorization": "Bearer invalid_token"})
